...
deploy-usb-remote flake network_target block_device:
    ssh root@{{network_target}} nix run 'github:nix-community/disko/latest#disko-install' -- --extra-files /root/.config/sops/age/keys.txt --flake 'github:danielgomez3/nix-config#{{flake}}' --disk main {{block_device}}
