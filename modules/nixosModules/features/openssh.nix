...
in
{
  ...
  users.users.${username}.openssh.authorizedKeys.keys = myHelper.findPublicUserSshKeys "${self.outPath}/hosts";
  ...
  users.users.root.openssh.authorizedKeys.keys = myHelper.findPublicUserOrRootSshKeys "${self.outPath}/hosts";
  services.openssh = {
    enable = true;
    # hostKeys = [
    #   {
