...
in
{
  ...
  users.${username}.openssh.authorizedKeys.keys = myHelper.listOfPublicUserOrRootSshKeys "${self.outPath}/hosts";
  ...
  users.root.openssh.authorizedKeys.keys = myHelper.listOfPublicUserSshKeys "${self.outPath}/hosts";
  services.openssh = {
    enable = true;
    # hostKeys = [
    #   {
